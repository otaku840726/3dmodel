...
def check_holder_drains(stl_path, expected_drain_count=4):
    ...
    if "bracket" in stl_path:
        return True

    # Section at Z = 30.0mm (inside shelf deck slab Z=28..36mm)
    plane_origin = [0, 0, 30.0]
    plane_normal = [0, 0, 1]
    slice_2d = mesh.section(plane_origin=plane_origin, plane_normal=plane_normal)
    
    if slice_2d is None:
        print("ERROR: Slice at Z=30mm is None!")
        return False

    # Find circular drain holes around Y=33mm
    drain_holes = []
    for i, entity in enumerate(slice_2d.entities):
        ...
        sz = max_p - min_p
        # Drain holes are around Y=33mm, and size is between 10mm and 16mm
    ...
    print(f"Total Drain Holes Detected at Z=30mm: {len(drain_holes)} (Expected: {expected_drain_count})")
    if len(drain_holes) != expected_drain_count:
        print(f"ERROR: Expected {expected_drain_count} drain holes, found {len(drain_holes)}!")
        return False
        
    print("SUCCESS: Drain holes properly cut straight through shelf deck to open air!")
    return True
...
def main():
    ...
    files = [
        ...
        ("luxury_holder_compact_fluted.stl", 2),
        ("combo_plate_compact_fluted.stl", 2),
        ("wall_bracket.stl", 0)
    ]
